...
export default function App() {
  ...
  if (screen === SCREEN.EDITOR) {
    return (
      <Shell game={game} lives={lives} showHud={false}>
        <Editor
          game={game}
          onSave={saveEditor}
  ...
  if (screen === SCREEN.VICTORY) {
    return (
      <Shell game={game} lives={lives} showHud>
        <Victory
          game={game}
          lives={lives}
  ...
  if (screen === SCREEN.GAMEOVER) {
    return (
      <Shell game={game} lives={0} showHud>
        <GameOver
  ...
  return (
    <Shell game={game} lives={lives} showHud onExit={() => setScreen(SCREEN.BRIEFING)}>
      {sub === 'intro' ? (
        <LevelIntro
          level={current.level}
...
function Shell({ game, lives, showHud, onExit, children }) {
  return (
    <div className="app">
      <header className="topbar">
        ...
        {showHud && (
          <div className="hud">
            <Hearts lives={lives} max={game.lives} />
            {onExit && (
